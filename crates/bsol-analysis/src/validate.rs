...
use bsol_syntax::{BsolAssignment, BsolBlock, BsolDocument, BsolItem, BsolListItem, BsolValue};
            ...
            BsolItem::Block(nested_block) => {
                if let Some(nested_rule) = rule.nested_rule_for_kind(&nested_block.kind) {
                    ...
                    *nested_counts.entry(nested_rule.id.clone()).or_default() += 1;
                    ...
                    nested.push(ValidatedBlock {
                        span: nested_block.span,
                        rule_id: nested_block.kind.clone(),
                        kind: nested_block.kind.clone(),
                        label: nested_block.label.as_ref().map(|q| q.value.clone()),
                        fields: HashMap::new(),
                        field_spans: HashMap::new(),
                        extras: HashMap::new(),
                        nested: Vec::new(),
                        lists: HashMap::new(),
                        raw_body: nested_block.schemaless_body.clone(),
                    });
                } else {
                    return Err(BsolError::schema_at(
                        nested_block.span,
